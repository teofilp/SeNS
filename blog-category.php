...
<body>
	...
	<div class="blog-div">
		...
		<?php 
			...
			$postQuery = $database->prepare("SELECT * FROM posts  WHERE post_category_id = :categoryId");
			$postQuery->bindParam(":categoryId", $category);
			$postQuery->execute();

			while($row = $postQuery->fetch(PDO::FETCH_ASSOC)) {

				echo "<div class='blog-post'><div class='post-image'><img src='images/" . $row['post_image'] . "' class='post-image-img'></div><div class='post-content'><a href='blog-post.php?postid=" . $row['id'] . "'><h2 class='post-title'>" . $row['post_title'] . "</h2></a><h6 class='date-category'>de " . $row['post_author'] . "</h6><div class='blog-post-text'><p class='post-text'>" . $row['post_content'] . "</p></div><a href='blog-post.php?postid=" . $row['id'] .  "&postcategory=" . $row['post_category_id'] . "'>Read more..</a></div></div>";     
			}
		 ?>
	</div>
